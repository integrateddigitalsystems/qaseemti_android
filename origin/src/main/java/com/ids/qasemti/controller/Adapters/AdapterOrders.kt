...
class AdapterOrders(val items: ArrayList<ResponseOrders>, private val itemClickListener: RVOnItemClickListener, context: Context) :
    ...
    override fun onBindViewHolder(holder: VHItem, position: Int) {
        ...
        try{holder.expected.text = items.get(position).deliveryDate}catch (e:Exception){}
        try{holder.orderDate.text = items.get(position).date}catch (e:Exception){}
        ...
        try{holder.tvLocation.setColorTypeface(con,R.color.redPrimary,"",false)}catch (e:Exception){}
